# Made changes to CAT_gPoE section in both functions
# Ready for 3d
    ...
    def __init__(self, hiddim_v=None, hiddim_p=None, op='CAT'):
        ...
        if op == 'CAT' or op == 'CAT_PoE':
            self.net1_mean_vis = nn.Sequential(
                weight_norm(nn.Conv3d(hiddim_v * 2, hiddim_v, 3, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_v, hiddim_v, 3, 1, 1))
            )
            ...
            self.net1_mean_pos = nn.Sequential(
                weight_norm(nn.Conv3d(hiddim_p * 2, hiddim_p, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_p, hiddim_p, 1, 1))
            )
            ...
            self.net_vis = nn.Sequential(
                weight_norm(nn.Conv3d(4 * hiddim_v, hiddim_v, 3, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_v, hiddim_v, 3, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_v, 2 * hiddim_v, 3, 1, 1))
            ...
            self.net_pos = nn.Sequential(
                weight_norm(nn.Conv3d(4 * hiddim_p, hiddim_p, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_p, hiddim_p, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_p, 2 * hiddim_p, 1, 1))
            ...
            self.gates_v = nn.Sequential(
                weight_norm(nn.Conv3d(hiddim_v * 4, hiddim_v * 4, 3, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_v * 4, hiddim_v * 4, 3, 1, 1))
            )
            self.gates_p = nn.Sequential(
                weight_norm(nn.Conv3d(hiddim_p * 4, hiddim_p * 4, 3, 1, 1)),
                nn.ELU(inplace=True),
                weight_norm(nn.Conv3d(hiddim_p * 4, hiddim_p * 4, 3, 1, 1))
            )
    ...
    def forward(self, x, y, mode, lognormal=False):  # -> x describe y
        if mode == 'vis':
            if self.op == 'CAT_PROD':
                ...
                y_var = -torch.log(torch.exp(mlogvar1) + torch.exp(mlogvar2))
                y_mean = torch.exp(y_var) * (torch.exp(mlogvar1) * mu1 + torch.exp(mlogvar2) * mu2)
                # if torch.isnan(y_var).any() or torch.isnan(y_mean).any():
                #     print('HEYYYYYYY')
                #     import IPython;
                #     IPython.embed()
            elif self.op == 'CAT':
