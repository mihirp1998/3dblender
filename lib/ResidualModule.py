...
class ResidualBlock(nn.Module):
    def __init__(self, resample, nonlinear, ifgate, indim, outdim, normalize):
        ...
        if ifgate:
            self.conv1 = nn.Conv3d(indim, outdim, 3, 1, 1)
            self.conv2 = nn.Conv3d(indim, outdim, 3, 1, 1)
            self.c = nn.Sigmoid()
            self.g = nn.Tanh()
    ...
    def conv(self, name, indim, outdim, normalize=None):
        if name == 'sconv_d':
            if normalize == 'weight_norm':
              return weight_norm(nn.Conv3d(indim, outdim, 4, 2, 1))
            else:
              return nn.Conv3d(indim, outdim, 4, 2, 1)
        elif name == 'sconv':
            if normalize == 'weight_norm':
              return weight_norm(nn.Conv3d(indim, outdim, 3, 1, 1))
            else:
              return nn.Conv3d(indim, outdim, 3, 1, 1)
        elif name == 'upconv':
            if normalize == 'weight_norm':
              return weight_norm(nn.ConvTranspose3d(indim, outdim, 4, 2, 1))
            else:
              return nn.ConvTranspose3d(indim, outdim, 4, 2, 1)
        else:
