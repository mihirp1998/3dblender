...
from lib.utils import get_tree_text, get_tree_bboxes
...
# Added new config params, changed CLEVRTREE() call
# Ready for 3d
...
def main():
    ...
    with open(configs.class_map) as f:
    ...
    # Modify configs 
    if configs.loss == 'cross_entropy' or configs.loss == 'binary_entropy':
        configs.maskweight = -1.0

    # loaders
    if 'CLEVR' in configs.data_folder:
        # we need the module's label->index dictionary from train loader
        if configs.input_mode == 'feature':
            file_format = 'npy'
        else:
            file_format = 'schematic'

        train_loader = CLEVRTREE(class_mapping, configs.loss, phase='train',
                                 base_dir=osp.join(configs.base_dir, configs.data_folder),
                                 class_count=configs.class_count, 
                                 batch_size=configs.batch_size,
                                 random_seed=configs.seed, shuffle=True, file_format=file_format)
        test_loader = CLEVRTREE(class_mapping, configs.loss, phase='test',
                                 base_dir=osp.join(configs.base_dir, configs.data_folder),
                                 class_count=configs.class_count, 
                                 batch_size=configs.batch_size,
                                 random_seed=configs.seed, shuffle=False, file_format=file_format)
        # gen_loader = CLEVRTREE(class_mapping, phase='test', base_dir=osp.join(configs.base_dir, configs.data_folder),
        #                          class_count=configs.class_count, 
        #                        batch_size=configs.batch_size,
        ...
        test_loader = CLEVRTREE(class_mapping, phase='test', base_dir=osp.join(configs.base_dir, configs.data_folder),
                                 class_count=configs.class_count, 
                                batch_size=configs.batch_size,
                                random_seed=configs.seed, shuffle=False)
    ...
    # hack, parameter
    im_size = train_loader.im_size[2]

    # model
    model = PNPNet(writer_sigmoid=configs.writer_sigmoid, only_tree=configs.only_tree, input_mode=configs.input_mode, input_feature_dim=configs.input_feature_dim, dual_recon=configs.dual_recon, hiddim=configs.hiddim, latentdim=configs.latentdim,
                   word_size=[configs.latentdim, configs.word_size, configs.word_size, configs.word_size], pos_size=[8, 1, 1, 1],
                   class_count=configs.class_count, nres=configs.nr_resnet, nlayers=4,
    ...
    print('Total parameter count is: {}'.format(count_parameters(model)))
    ...
    if configs.checkpoint is not None and len(configs.checkpoint) > 0:
        ...
        epoch_offset = int(configs.checkpoint[-10:].split('.')[0].split('_')[-1])
        print('setting epoch offset to {}'.format(epoch_offset))
    else:
        model.apply(weights_init)
        epoch_offset = 0

    if configs.mode == 'train':
        train(model, train_loader, test_loader, gen_loader, epoch_offset, configs=configs)
    elif configs.mode == 'test':
        print(exp_dir)
        print('Start generating...')
        epoch_num = configs.checkpoint.split('/')[-1].split('.')[0].split('_')[-1]
        generate(model, gen_loader=test_loader, num_sample=2, target_dir=exp_dir, epoch_num=epoch_num)
...
def train(model, train_loader, test_loader, gen_loader, epoch_offset, configs):
    model.train()
    ...
    for epoch_num in range(0, configs.epochs + 1):
        ...
        trainer.train_epoch(epoch_num + epoch_offset, timestamp_start)
        if epoch_num % configs.validate_interval == 0:
            minloss = trainer.validate(epoch_num + epoch_offset, timestamp_start, minloss)
        ...
        if epoch_num % configs.save_interval == 0 and epoch_num > 0:
            torch.save(model.state_dict(),
                       osp.join(configs.exp_dir, 'checkpoints', 'model_epoch_{0}.pth'.format(epoch_num + epoch_offset)))


def generate(model, gen_loader, num_sample, target_dir, epoch_num):
    model.eval()
    model.cuda()
    ...
    while epoch_end is False:
        ...
        with torch.no_grad():
            ...
            for j in range(num_sample):
                sample, prior_mean, prior_var = model.generate(data, trees)
                if not batch_size:
                    batch_size = sample.size(0)

                for t in trees:
                    text = get_tree_text(t)
                    box = get_tree_bboxes(t)
                    inputs_tree.append(text)
                    bboxes.append(box)
                inputs.append(data.cpu().data.numpy())
                outputs.append(sample.cpu().data.numpy())
                pmeans.append(prior_mean.cpu().data.numpy())
                pvars.append(prior_var.cpu().data.numpy())
                model.clean_tree(trees)


    epoch_result_dir = osp.join(target_dir, 'generated-samples', 'epoch-{}'.format(epoch_num))
    ...
    pmeans = np.vstack(pmeans)
    pvars = np.vstack(pvars)
    np.savez(osp.join(epoch_result_dir, 'generated_voxels.npz'), inp=inputs, out=outputs, pmeans=pmeans, pvars=pvars)
    with open(osp.join(epoch_result_dir, 'trees.pickle'), 'wb') as f:
        pickle.dump(inputs_tree, f, protocol=2)
    with open(osp.join(epoch_result_dir, 'bboxes.pickle'), 'wb') as f:
        pickle.dump(bboxes, f, protocol=2)
