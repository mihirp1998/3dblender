...
from lib.utils import color_grid_vis, AverageMeter, get_tree_text

# Made changes to sample(), _total(), train_epoch()
...
class PNPNetTrainer:
    ...
    def train_epoch(self, epoch_num, timestamp_start):
        ...
        while epoch_end is False:
            ...
            recon = modelout
            l1, l2, l3 = rec_loss.cpu().data.numpy()/data.size(0), kld_loss.cpu().data.numpy()/data.size(0), pos_loss.cpu().data.numpy()/data.size(0)
            # l1, l2, l3 = rec_loss.cpu().data.numpy(), kld_loss.cpu().data.numpy(), pos_loss.cpu().data.numpy()
            ...
            # loss = rec_loss + self.configs.pos_beta * pos_loss
            # loss = rec_loss
            loss.backward()
            self.optimizer.step()
            ...
            if batch_idx % 30 == 0:
            #     # CHECK https://stackoverflow.com/questions/4348733/saving-interactive-matplotlib-figures
            ...
            # print('Epoch:{0}\tIter:{1}/{2}\tRecon {3:.6f}\t KL {4:.6f}\t Pos {5:.6f}'.format(epoch_num, batch_idx,
            #                                                                    len(self.train_loader) // self.configs.batch_size,
            #                                                                    train_rec_loss.batch_avg, train_kld_loss.batch_avg, train_pos_loss.batch_avg))
    ...
    @staticmethod
    def _total(tensor):
        return tensor.size(0) * tensor.size(1) * tensor.size(2) * tensor.size(3) * tensor.size(4)
    ...
    def validate(self, epoch_num, timestamp_start, minloss):
        ...
        dumping_count = 40
        inputs = np.vstack(inputs)[:dumping_count]
        outputs = np.vstack(outputs)[:dumping_count]
        np.savez(osp.join(epoch_result_dir, 'generated_voxels.npz'), inp=inputs, out=outputs)
        ...
        print('====> Epoch: {}  Test rec loss: {:.6f} Test kld loss: {:.6f} Test pos loss: {:.6f}'.format(
            epoch_num, losses[0], losses[1], losses[2]))
        print('Elapsed time:', elapsed_time)
